...
public class Problem3Kmean4 {
	// private static final Log LOG =
	// LogFactory.getLog(Problem2.class.getName());
	private static final Logger logger = Logger.getLogger(Problem3Kmean4.class
			.getName());
	...
	public static class MyCombiner extends
		...
		public void reduce(IntWritable key, Iterable<Text> values,
			...
			context.write(key, new Text((int)new_x + "," + (int)new_y));
		}
	}
		...
		public void reduce(IntWritable key, Iterable<Text> values,
			...
			for (Text val : values) {
				if (val.toString().contains(",")) {
					...
					totalPoint += Integer.parseInt(val.toString().replaceAll("\\s", ""));
				}
			}
			// average here
			new_x = (int)new_x / totalPoint;
			new_y = (int)new_y / totalPoint;
	...
	public static void run(String[] args) throws Exception {
		...
		for (int i = 0; i < max_iter; i++) {
			...
			job.setJarByClass(Problem3Kmean4.class); // change the class here
